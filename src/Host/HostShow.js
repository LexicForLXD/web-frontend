...
class HostShow extends Component {
  ...
  render() {
    return (
      <div>
        {this.state.redirect && (
          <Redirect from="/hosts/show" exact to="/hosts" />
        )}
        {this.state.notFound && <h6>Host not found!</h6>}
        {!this.state.notFound && (
          <div>
            ...
            >
              <i className="fa fa-edit" /> Edit Host
            </Button>
            {!this.state.host.authenticated && (
              <Button
                type="button"
              ...
              >
                <i className="fa fa-key" /> Authorize Host
              </Button>
            )}
            <Button
              type="button"
            ...
            >
              <i className="fa fa-trash" /> Delete Host
            </Button>
          </div>
        )}
        {!this.state.notFound &&
