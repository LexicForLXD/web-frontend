<template>
    <v-card v-if="backup">
        <v-toolbar>
            <v-toolbar-title>
                {{backup.manualBackupName}} - {{backup.timestamp}}
            </v-toolbar-title>
        </v-toolbar>
        ...
        <v-card-actions>
            <v-btn @click="onDelete" v-show="!restore">Delete</v-btn>
...
export default {
  computed: {
    ...
    destination() {
      return this.$store.getters.getBackupDestinationById(
        this.backup.destinationId
      );
    },

    destinationIndex() {
      return this.$store.getters.getBackupDestinationIndexById(
        this.backup.destinationId
      );
    },

    schedule() {
      return this.$store.getters.getBackupScheduleById(
        this.backup.backupScheduleId
      );
    },

    scheduleIndex() {
      return this.$store.getters.getBackupScheduleIndexById(
        this.backup.backupScheduleId
      );
    },
  ...
  methods: {
    ...
    getTarballs() {
      this.startLoading();
      restoreApi
        .getFiles(this.backup.id, this.hostId)
        .then(res => {
