...
class MonitoringShow extends Component {
  ...
  getChecks = () => {
    if (this.props.toggleContainers) this.httpGetContainerChecks();
    else this.httpGetHostChecks();
  };
  ...
  httpGetContainerChecks = () => {
    ...
    this.props.httpRequest(
      "GET",
      "monitoring/checks/containers/" + id,
      null,
      obj => {
  ...
  getGraphs = () => {
    this.setState({ graphs: [] }, () => {
      ...
      else this.state.hostChecks.forEach(hc => this.httpGetGraph(hc.id));
    });
  };
  ...
  httpGetGraph = checkId => {
    const path = this.props.toggleContainers
      ? `monitoring/checks/${checkId}/containers/graph?timerange=-${
  ...
  render() {
    return (
      <div>
        {this.state.notFound && <h6>No monitoring configuration found!</h6>}
        {!this.state.notFound && (
          <div>
            <form>
            ...
            {this.state.graphs.map(graph => (
              <div>
                <img
                  class="graph"
                  src={URL.createObjectURL(graph.blob)}
                  alt="graph"
                />
                <br />
              </div>
